...
use syn::{Lit, NestedMetaItem, MetaItem, AttrStyle};
...
fn get_packet_header(ast: &syn::MacroInput) -> PacketHeader {
  ...
  let kind = items.iter().filter_map(|item| get_key_value("kind", item)).next()
    .expect("#[derive(MuPacket)] attribute field 'kind' not valid");
  let code = items.iter().filter_map(|item| get_key_value("code", item)).next()
    .expect("#[derive(MuPacket)] attribute field 'code' not valid");
  let subcode = items.iter().filter_map(|item| get_key_value("subcode", item)).next();
  ...
  PacketHeader {
    ...
    subcode: subcode
      .map(|codes| codes.split("|")
        .map(|code| u8::from_str_radix(&code, 16)
          .expect("#[derive(MuPacket)] attribute field 'subcode' must be pipe-separated hex values."))
        .collect())
...
fn get_key_value(key: &str, item: &NestedMetaItem) -> Option<String> {
  match item {
    &NestedMetaItem::MetaItem(ref item) => {
      match item {
        &MetaItem::NameValue(ref name, ref literal) if name == key => {
          match literal {
