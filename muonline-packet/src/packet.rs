...
impl Packet {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Packet, io::Error> {
        Self::from_bytes_ex(bytes, None, None).map(|(packet, _, _)| packet)
    }

    /// Constructs a packet from an array of potentially encrypted bytes.
    pub fn from_bytes_ex(bytes: &[u8],
                         cipher: Option<&[u8]>,
        ...
        let (size, original_size, crypto_count) = if kind.is_encrypted() {
            if let Some(decryption) = decryption {
                ...
                return Err(io::Error::new(io::ErrorKind::Other, "missing decryption for packet"));
            }
        } else {
            (size, size, None)
        };
        ...
        if packet.code() != XOR_SKIP_CODE {
            if let Some(cipher) = cipher {
                ...
                Self::xorcrypt(cipher, packet.kind(), packet.code(), &mut packet.data, iter.rev())
            }
        }
    ...
    pub fn to_bytes_ex(&self, cipher: Option<&[u8]>, encryption: Option<(&Crypto, u8)>) -> Vec<u8> {
        ...
        if let Some((_, crypto_counter)) = encryption {
            ...
            bytes.write_uint::<BigEndian>(self.len() as u64, self.kind().bytes()).unwrap();
        }
        ...
        if let Some((crypto, _)) = encryption {
            ...
            bytes.write_uint::<BigEndian>(size as u64, kind.bytes()).unwrap();
