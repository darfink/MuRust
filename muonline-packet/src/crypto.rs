...
impl Crypto {
    ...
    pub fn new(enc: &[u8; ENCRYPTION_SIZE],
               dec: &[u8; ENCRYPTION_SIZE],
               xor: &[u32; 4]) -> Self {
        Crypto {
    ...
    fn convert_11to8_bytes(&self, out: &mut [u8], slice: &[u8]) -> Result<usize, io::Error> {
        ...
        let xor = writer.into_inner().iter().fold(0xF8, |xor, &value| xor ^ value);
        if finale[1] == xor {
            Ok(finale[0] as usize)
        } else {
            Err(io::Error::new(io::ErrorKind::InvalidData, "Incorrect data hash"))
        }
    }
...
mod tests {
    ...
    fn client2server() {
        ...
        assert_eq!(enc, [0xE3, 0xB3, 0x53, 0x9A, 0x4F, 0xC8, 0x32, 0x7D, 0x04, 0x37, 0x0F]);
    ...
    fn server2client() {
        ...
        assert_eq!(enc, [0x47, 0x93, 0x15, 0x3B, 0x0B, 0x1C, 0x15, 0x7C, 0x16, 0x37, 0x0F]);
    ...
    #[test]
    fn large_buffer() {
        let raw = [0x7C, 0xE7, 0xE6, 0xA2, 0x1E, 0xA8, 0xDA, 0xBC,
                   0xDB, 0x6D, 0x31, 0x62, 0xFE, 0xA7, 0xA0, 0xF3,
                   0xF4, 0x05, 0x1D, 0x64, 0x1A, 0x42, 0xC2];
