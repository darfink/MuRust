...
impl<T> PacketEncodable for T where T: PacketType + Serialize {
...
impl<T> PacketDecodable for T where T: PacketType + DeserializeOwned {
    ...
    fn from_packet(packet: &Packet) -> Result<Self, io::Error> {
        if packet.kind() == T::kind() && packet.code() == T::CODE {
            ...
            if subcodes.len() <= packet.data().len() {
                if subcodes.iter().zip(packet.data().iter()).all(|(x, y)| x == y) {
        ...
        Err(io::Error::new(io::ErrorKind::Other, "codes differ from the type's"))
    }
}
