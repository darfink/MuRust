...
pub use self::integer::{IntegerLE, IntegerBE};
pub use self::string::{StringFixed, StringFixedTransform, StringTransform};
pub use self::vector::{VectorLengthLE, VectorLengthBE};
...
mod tests {
    ...
    fn binary() {
        ...
        let foo_dez: Foo = bincode::config().native_endian().deserialize(&data).unwrap();
