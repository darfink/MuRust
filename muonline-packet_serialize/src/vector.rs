...
use serde::{self, Serialize, Serializer, Deserializer};
...
impl<Length, Endian> VectorLength<Length, Endian>
        where Length: DeserializeOwned + Serialize + PrimInt,
...
impl<'de, T, Length, Endian> serde::de::Visitor<'de> for VectorLengthVisitor<T, Length, Endian>
    ...
    fn visit_seq<A>(self, mut seq: A) -> Result<Self::Value, A::Error> where A: serde::de::SeqAccess<'de> {
        ...
        let size = Endian::to_endian(size)
            .to_usize()
            .ok_or(serde::de::Error::custom("invalid value, not usize compatible"))?;
        ...
        if data.len() != size {
            Err(serde::de::Error::invalid_length(data.len(), &format!("a length of {}", size).as_str()))
        } else {
            Ok(data)
        }
    }
