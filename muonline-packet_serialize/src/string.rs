...
pub struct StringFixedTransform<Size: Unsigned, Trans: StringTransform>(PhantomData<Size>, PhantomData<Trans>);

/// A string with a fixed length.
pub type StringFixed<S> = StringFixedTransform<S, NoTransform>;

impl<'de, Size: Unsigned, Trans: StringTransform> serde::de::Visitor<'de> for StringFixedTransform<Size, Trans> {
    type Value = String;
    ...
    #[inline]
    fn visit_seq<A>(self, mut seq: A) -> Result<String, A::Error> where A: serde::de::SeqAccess<'de> {
...
impl<Size: Unsigned, Trans: StringTransform> StringFixedTransform<Size, Trans> {
    ...
    pub fn serialize<S>(string: &String, serializer: S) -> Result<S::Ok, S::Error>
          where Size: Unsigned, S: Serializer {
        let size = Size::to_usize();
    ...
    pub fn deserialize<'de, D>(deserializer: D) -> Result<String, D::Error>
          where Size: Unsigned, D: Deserializer<'de> {
        deserializer.deserialize_tuple(Size::to_usize(), StringFixedTransform::<Size, Trans>(PhantomData, PhantomData))
    }
}
